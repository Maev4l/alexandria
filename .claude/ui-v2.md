...
- Deployed on AWS via Terraform (`packages/infrastructure`: `webclient.tf` S3 bucket + `cloudfront.tf` distribution). Deploy with `make frontend-deploy` (build → `s3 sync` → CloudFront invalidation).
...
└─────────────────────────────────────────────────────────────┘        │
                                                                       │
                  ◄────────────────────────────────────────────────────┘
                        (Cancel from AddBook/AddVideo → LibraryContent)
