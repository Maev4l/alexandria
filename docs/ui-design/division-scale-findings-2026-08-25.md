...
The division scale states it plainly: *gaps between blocks are whole divisions, always*, and the
division is 8px. It also permits optical adjustments of 1–3px **inside** a plate, a tag or a stamp —
so a typographic interior is not bound by the rule and a gap between blocks is.
...
than a hand-tuned bracket value like `PlateLine`'s `mt-[3px]`, which is the shape the optical
allowance for plate and tag interiors is written for.
...
`pb-3` is 12px. It is padding before a structural rule (`border-b-2`), which the division scale binds to whole
...
This is the same shape as the type-scale guard's own history, recorded in the typography rules: a
