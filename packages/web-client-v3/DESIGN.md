...
taking the rest of the column — `min-h-0 flex-1 overflow-y-auto`, marked `data-scroll-region`. The
header never moves; the content travels under it. `min-h-0` is the load-bearing half: without it a
flex child refuses to shrink below its content, the column grows past the viewport, and the document
scrolls instead — which is the shape four screens had.

**Every screen has it but one, and the one is a deliberate variant rather than a leftover.** `Libraries`, `LibraryBrowse` and `UnshareLibrary` had it from the beginning —
the first two because `PullToRefresh` *is* a scroll region and they were built around it — joined
by `AddVideo`, which needs a fixed height for a different reason (its viewfinder yields rather than
pushing the manual escape off-screen). `ItemDetail`, `Search`, `BookDetectionResults` and
`VideoDetectionResults` were `min-h-dvh` with document scroll, so their headers scrolled away with
the content, and they were changed to match; then `EditItem`, `NewBook` and `NewVideo`, the sharpest
case of the lot, since a long form whose Save sits at the bottom is where a travelling header costs
most; then the remaining eleven in one pass — auth, settings, the library and collection forms,
`AddBook`, `PendingApproval` and `ItemHistory`.

**The exception is `AddVideo`, and it is the only page in the app with no scroll region at all.** It
is fixed-height for a different reason: its viewfinder yields, shrinking so the manual escape stays
on screen rather than letting the page grow. A page that must not scroll and a page that scrolls
inside a region are both fixed-height columns, and only the second has a `data-scroll-region`.

**These counts were wrong twice before they were right, and the second time the *re-derivation* was
wrong** — `grep -l 'h-dvh flex-col'` also matches `min-h-dvh flex-col`, which is a growing column
rather than a fixed one, so `PendingApproval` was counted as converted while it still scrolled the
document. A number in prose is the one claim here a commit elsewhere can falsify without opening the
file, and a sloppy predicate falsifies it without any commit at all. Match `h-dvh` as a whole word:
`grep -LE 'className="[^"]*\bh-dvh\b[^"]*flex-col' src/pages/*.jsx` lists what is left.

**Neither group was wrong against any rule, because until now there was no rule.** The divergence
was invisible in review for as long as this section did not exist, and it was reported by the owner
rather than found by anything here. `UnshareLibrary` is the sharpest evidence: it had the shape and
its region was **unmarked**, so it would have escaped the sheet lock silently had a sheet ever
opened over it — and it was found by widening a check's route list, not by the check.

Three things follow from it, and each is a consequence rather than a preference:

- **A sticky child sticks to the region, not the viewport.** `IndexLetter`'s `top-0` has always
  come to rest under LibraryBrowse's header for this reason, and the search field's `top-0` now
  does the same with no offset to compute. A screen that scrolls the document would need the
  header's height, and the header's height is not a constant — it grows with the safe-area inset
  and again when it carries the search field. **Not needing that number is the point.**
- **The region's bottom edge is the viewport's**, so a bottom-anchored control has no over-scroll
  to save it. A screen that ends in its region carries `pb-[max(…,env(safe-area-inset-bottom))]` on
  the region; one that ends in a bar below the region — `Libraries`, `UnshareLibrary` — puts
  `pad-bottom-safe` on the bar instead, and the region needs none. **`LibraryBrowse` has neither**,
  so the last row of a thousand-item stream sits under the home indicator on an installed PWA. That
  is a real gap, recorded rather than quietly fixed because it is nobody's ask; it predates this
  section and is the only screen with the shape that breaches it.
- **An absolutely-positioned descendant escapes the region unless something anchors it**, and a
  fixed-height column is what makes that visible. `sr-only` is `position: absolute`, so with no
  positioned ancestor its containing block is the initial one — outside the scroll region, and
  therefore not clipped by it. `PlateButton`'s disabled `reason` was exactly that: a 1px box landing
  below the viewport, lengthening `documentElement.scrollHeight`, so the page gained a slice of
  document scroll and the pinned header travelled off the top after all — 124px on `NewBook`, 420px
  on `NewVideo`, whose longer form puts the disabled submit lower. `PlateButton` now sets `relative`
  and that class is load-bearing, not decoration. **The general form: on a shell-shaped screen,
  anything `absolute` needs a positioned ancestor inside the region**, and the check that catches it
  is *the document itself never scrolls* rather than anything about the element.
- **`document.body`'s overflow governs nothing here.** Anything that needs to stop the page —
  today only `Sheet`'s lock — must freeze the regions themselves, which is what the attribute is
  for. A browser check asserts the converse: any element whose computed overflow scrolls must
  carry the attribute — read off the computed style, not a class name — so a surface nobody marks
  fails rather than silently escaping the lock. **It must not also require the element to be
  overflowing right now**, which is how its first version came to pass against a deliberately
  unmarked region: two fixture recipients do not fill a 667px viewport, so it would have stayed
  quiet until real data grew past the screen — that is, until the thing was already scrolling
  behind somebody's sheet. A region is a region whether or not today's fixture fills it.
  Two things are exempt, both of them not page surfaces: the sheet's own panel, which scrolls
  internally, and any native form control — a `<textarea>` computes `overflow-y: auto` by default,
  so without that exemption the relaxation above made all three item forms fail on a correct
  `SUMMARY` field. **Relaxing a guard trades a blind spot for a false positive, and the second is
  the more dangerous of the two**: a check whose first act is to report correct code gets deleted
...
| **Marks** | The few universal affordances left after the no-icons-for-state rule — back, add, search, close and the like. **Hand-authored SVG at 2px stroke on the division scale; no icon library.** An imported set would arrive at someone else's stroke weights and optical sizes and quietly break the rule system, to carry nine: back, the select's chevron, search in two cuts — 16 for the pinned field, 20 for the header plate on the cover — add, close, check, copy and the reveal pair. **All nine are used.** A camera and a three-dot mark were authored and imported by nothing, and both are gone — the file exports exactly the marks the app draws, no more. The row-actions affordance is not among these at all — its three squares are boxes rather than a drawn glyph, and Row Actions below owns them. State is never a mark; state is a stamp, a ribbon or a rule. **A Mark carries no colour of its own**: every one is stroked in `currentColor` at 2px with no fill, `aria-hidden` and out of the tab order, so the control around it owns both the colour and the accessible name. The shared base sets no `stroke-linecap`, so every mark renders at the SVG default of `butt` — the file's own header comment says so now, rather than claiming *square caps*, which was true only of the three-dot mark that has since been removed. The back Mark is the case worth recording, because it is two colours: **`--ink-soft` on paper**, where it is a quiet mark serving content rather than one of the structural rules that stay at full strength, and **`--imprint` when the header is inverted**, where yellow on `--ink` clears 11.71:1 and describes a form perfectly well. Both are permitted by the palette as it now reads and both were condemned by it as it read before — the quiet register was written as secondary content only, and the yellow law was stated palette-wide when the measurement behind it was a paper one. |
...
| Appending a page | `Loading more`, 11px caps in `--ink-soft`, centred, `role="status"`, **below the stream rather than in place of it**. **Three shapes share the word *loading*, and each is a different decision.** A **cold mount** empties the entries first, so the skeletons are all there is. An **append** draws no skeleton anywhere: the rows stay exactly as they are and one line beneath them says more is coming. A **pull-to-refresh** sets the same loading flag and does *not* empty the entries — only the mount does — so four skeletons draw **above** the rows still on screen, and the reader sees both at once until the response lands. That is deliberate: keep showing the reader what they already have rather than blanking a thousand-item stream onto bare paper for a request they already trust to succeed. When the response arrives it replaces the stream outright — a refresh loads with no token, and a load with no token starts from an empty array, so the old rows are not merged with the new ones, they are swapped for them in one motion. So the distinction is **not** *loading replaces content and appending does not* — refresh does both, in sequence: it keeps the old rows on screen while the request is in flight, then discards them for the real answer the instant it arrives. Pagination itself stays invisible — no page number, no control, no count. |
