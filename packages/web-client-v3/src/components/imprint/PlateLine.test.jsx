...
// render it. That was survivable while it had one shape and became a gap the moment §5's
...
describe('PlateLine', () => {
  ...
  describe('the browse row (default surface)', () => {
    ...
    it('renders nothing at all when the item carries neither', () => {
      // §4's accepted residual: a film entered by hand with no director and no year prints a bare
  ...
  // THE THIRD SURFACE. §5 already varies this line by surface — it gains the edition on item
  ...
  describe('the candidate surface', () => {
    ...
    it('leaves the browse row unchanged, with no runtime', () => {
      ...
      // comparison, and §4 withholds the runtime from it deliberately.
      const { container } = render(<PlateLine item={film} />);
      expect(container.textContent).toBe('Jean-Pierre Melville · 1967');
    });
    ...
    it('never prints an identifier for a book candidate', () => {
      ...
      // a row must never carry one (§4), and on that screen the scanned code already sits once at
